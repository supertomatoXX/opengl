...
public:
    ...
    virtual void RenderSceneCB()
    {
		...
		//一个有趣的性能技巧是，将天空盒子的渲染放到所有其他模型最后。因为，我们知道天空盒子总是位于其他所有模型之后的，
		//一些GPU会有优化机制使得在执行片段着色器之前就可以进行早期的深度测试，并丢弃那些测试失败的片元，这样对于提高天空盒子的渲染效率是很有用的，
		//因为只需要对那些没被其他模型所覆盖的背景图元执行着色器即可。但是为了生效，我们必须获取封装了所有Z值的深度缓冲，那样当天空盒子要渲染时所有要用到的信息就都已经准备好了。

		//深度测试预优化，fs之前。shader有改深度值，做alpha测试等的情况，破坏深度测试时，则会破坏深度预测试。
        m_pSkyBox->Render();
