...
public:
    ...
    {   
		...
		//这里将每个光源分开计算是因为当某个像素的模板值大于 0 时，无法确定其是因为哪一个光源导致的，要分开计算。

		//对每个光源的光照计算结束之后先禁用模板测试，因为对于平行光源来说我们需要对每个像素都进行光照计算。
		...
		//当进入点光源阶段之后需要从深度缓存中获取数据，这样就产生了一个问题——如果直接将图像渲染到默认的 FBO 中那么就没有权限去访问 GBuffer 中的深度缓存。
		//但是 GBuffer 必须拥有自己的深度缓存,因为想 GBuffer 中渲染场景时,也无法访问默认 FBO 的深度缓存。因此解决方案就是在 GBuffer 的 FBO 中添加一个颜色缓存来存放最终渲染的结果，
	...
	//虽然在之后的点光源阶段会再次禁用深度测试，但是这里由于模板操作依赖于深度测试，所以还需要它。
	//之后禁用背面剔除，因为对于光源体来说，希望对其正反面都进行处理。
	//在这之后清空模板缓存并将模板测试设置为 “始终通过”，并根据 “背景” 中的描述为其设置模板操作。
	//之后的操作就很平常了——根据光源的参数渲染光源的包围球，渲染完成之后模板缓存中只有被真正处于光源体内部的对象所覆盖的像素才会存放正值。
	//这样我们就可以对这些像素进行光照计算了。
